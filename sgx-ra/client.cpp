...
int main (int argc, char *argv[])
{
    ...
    */
    ...
    // shawn233: change sample client to cloud service provider in our scenario  
    if ( flag_stdio ) {
		msgio= new MsgIO();
	} else {
		try {
    ...
    while (msgio->server_loop()) {
        if (!read_user_init()) {
            eprintf("error reading user initial message\n");
            goto disconnect;
        }

        fprintf(stderr, "[service] sending msg 0 and msg 1..\n");
        do_attestation(eid, &config);
...
int read_user_init() {
    ...
    fprintf(stderr, "[service] waiting for user initial message\n");
    rv = msgio->read((void **) &user_init, NULL);
    if ( rv == -1 ) {
    ...
    fprintf(stderr, "[service] starting attestation with user id 0x%x\n", user_init->user_id);
    return 1;
}
...
int do_attestation (sgx_enclave_id_t eid, config_t *config)
{
    ...
    */
    ...
    size_t data_block_sz = 0;
    rv = msgio->read((void **) &data_block, &data_block_sz);
    if ( rv == 0 ) {
		enclave_ra_close(eid, &sgxrv, ra_ctx);
		fprintf(stderr, "protocol error reading data block\n");
		delete msgio;
		exit(1);
	} else if ( rv == -1 ) {
		enclave_ra_close(eid, &sgxrv, ra_ctx);
		fprintf(stderr, "system error occurred while reading data block\n");
		delete msgio;
		exit(1);
	}

    edividerWithText("Data Block Sent from User");
    eprintf("[service] data block received from user\n");
    eprintf("length: %u\n", data_block_sz);
    eprintf("content: %s\n", data_block);
