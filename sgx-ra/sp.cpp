...
int main(int argc, char *argv[])
{
    ...
    fprintf(stderr, "[user] attestation starts\n");
    ...
    fprintf(stderr, "[user] log file created\n");
    ...
    fprintf(stderr, "[user] arguments parsed\n");
    ...
    */
    ...
    /* shawn233: Remaining argument is host[:port] */

    fprintf(stderr, "[user] setting hostname and port of cloud service provider...\n");
	if ( flag_stdio && argc ) {
        fprintf(stderr, "[user] branch 0\n");
        usage();
    }
	else if ( !flag_stdio && ! argc ) {
		// Default to localhost
        fprintf(stderr, "[user] branch 1\n");
    ...
    if (config.port)
        port = strdup(config.port); // shawn233: duplicate port values to ensure compatibility with the original codes

    fprintf(stderr, "[user] cloud service provider hostname and port set up\n");
    ...
    fprintf(stderr, "[user] ca bundle done\n");
	...
	crypto_init();
    fprintf(stderr, "[user] crypto init\n");
   ...
   // shawn233: user connects to cloud service provider
   fprintf(stderr, "[user] connecting to cloud service provider...\n");
   if ( config.server == NULL ) {
		msgio = new MsgIO();
	} else {
		try {
    ...
    fprintf(stderr, "[user] connection to cloud service provider established\n");
    ...
    /* shawn233: but now we are not */
    /* shawn233: instead, the user intiates an attestation by sending initial message to cloud service provider */

    struct user_init_struct {
        uint32_t user_id;
    } user_init;
    user_init.user_id = 0x12345678; // shawn233: for test purposes. should be replaced in practices
    msgio->send((void *) &user_init, sizeof(user_init_struct));
    fsend_msg(fplog, (void*) &user_init, sizeof(user_init_struct));
    fprintf(stderr, "[user] attestation user inital message sent\n");
    ...
    sgx_ra_msg1_t msg1;
    sgx_ra_msg2_t msg2;
    ra_msg4_t *msg4 = (ra_msg4_t *)malloc(sizeof(ra_msg4_t));
    ...
    */
    ...
    // shawn233: report attestation results on user side rather than sending report to cloud service provider
    // this part was copied from client.cpp
    edividerWithText("Enclave Trust Status");
    
    int enclaveTrusted = NotTrusted;
    sgx_status_t status;
    ...
    // shawn233: note that operation to enclave will not be allowed on user side
    // since the encalve is on the service provider side
    // so the following function can not be implemented here
	/*
...
/* Windows implementation is not available yet */
        ...
        // shawn233: user may want to send some information to service provider after attestation
        char *data_block;
        data_block = strdup("some message to provider");
        eprintf("[user] data block size is %u\n", sizeof(char)*(strlen(data_block)+1));
        msgio->send((void *) data_block, sizeof(char)*(strlen(data_block)+1));
	} else {
