...
static sgx_status_t SGX_CDECL sgx_ecall_test_capstone_iter(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	ecall_test_capstone_iter();
	return status;
}

static sgx_status_t SGX_CDECL sgx_secure_timer(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	secure_timer();
...
static sgx_status_t SGX_CDECL sgx_main_ssa_init(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	main_ssa_init();
...
SGX_EXTERNC const struct {
	...
	5,
	{
		{(void*)(uintptr_t)sgx_ecall_receive_binary, 0},
		{(void*)(uintptr_t)sgx_ecall_receive_entrylabel, 0},
		{(void*)(uintptr_t)sgx_ecall_test_capstone_iter, 0},
		{(void*)(uintptr_t)sgx_secure_timer, 0},
		{(void*)(uintptr_t)sgx_main_ssa_init, 0},
	}
};

SGX_EXTERNC const struct {
	size_t nr_ocall;
	uint8_t entry_table[38][5];
} g_dyn_entry_table = {
	38,
	{
		{0, 0, 0, 0, 0, },
...
sgx_status_t SGX_CDECL ocall_sgx_time(time_t* retval, time_t* t)
{
	...
	if (t != NULL) {
		...
		__tmp_t = __tmp;
		if (memcpy_s(__tmp_t, ocalloc_size, t, _len_t)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
